package droidlol.aly.asphalt.ui;
...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_IMAGE && resultCode == Activity.RESULT_OK) {
            ...
            Picasso.with(Main2Activity.this).load(destination).fit().centerCrop().into(imageView);
            resultTextView.setText("Processing");

            AsyncTask.execute(new Runnable() {
                @Override
                public void run() {
                    String result = OpenALPR.Factory.create(Main2Activity.this, ANDROID_DATA_DIR).recognizeWithCountryRegionNConfig("au", "", destination.getAbsolutePath(), openAlprConfFile, 10);
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                if (results == null || results.getResults() == null || results.getResults().size() == 0) {
                                    Toast.makeText(Main2Activity.this, "It was not possible to detect the licence plate.", Toast.LENGTH_LONG).show();
